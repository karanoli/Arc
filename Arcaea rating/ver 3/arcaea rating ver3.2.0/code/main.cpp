...
inline void read_map(){
	fin.open("datas//map.txt",ios::in);mm.clear(); 
	while(getline(fin,s)){
		int n=s.size(),i=0,x=0;
		string name1="",name2="";
		if(s[i]=='<'){
			++i;
			while(i<n&&s[i]!='>') name1+=s[i],i++;
		}
		while(s[i]!='<'&&i<n)++i;
		if(s[i]=='<'){
			++i;
			while(i<n&&s[i]!='>') name2+=s[i],i++;
		}
		mm[name2]=name1;
	}
	fin.close(); 
}
map<node,int>gamedata;
inline void read_data(){
	fin.open("datas//playdata.txt",ios::in);gamedata.clear(); 
	while(getline(fin,s)){
		int n=s.size(),i=0,x=0;
		string name="",nd="";
		if(s[i]=='<'){
		...
		if(mm.count(name))name=mm[name];
		gamedata[node(name,nd)]=x;
	}
	fin.close(); 
}
...
inline void read_web(){
	fin.open("datas//site.txt",ios::in);
	while(getline(fin,s)){
		int n=s.size(),i=0;
		string name="",nd="" ,wz="";
		if(s[i]=='<'){
...
inline void input(){
	...
	if((isdigit(s[0])&&isdigit(s[1])&&s[2]=='.')||(isdigit(s[0])&&s[1]=='.')){
		...
		while(i<n&&s[i]!='.')ds=ds*10+(s[i]^48),++i;
		++i;ds=ds+(s[i]^48)*0.1;
		if(ds<8.0||ds>12.0){cout<<"The rating is not found!!!\n";Sleep(3000);goto letagain;}
		vector<node>:: iterator it=disrat[ds].begin();
		for(i=0;it!=disrat[ds].end();++it){
			if(++i<10)putchar(' '); 
			cout<<i<<". ["<<(*it).nd<<"] "<<(*it).name<<"\n";
		}
		cout<<"number ? or inpur @ to search again\n";
		getline(cin,s);i=0;n=s.size();x=0;
		if(s=="@")goto letagain;
		while(i<n&&s[i]!='.')x=x*10+(s[i]^48),++i;
		if(x>disrat[ds].size())goto letagain;
		xt=disrat[ds][x-1];
		cout<<disrat[ds][x-1].name<<"\n"; 
	}else{
		cout<<"PST or PRS or FTR or BYD or ETR\n";
	...
	if(gamedata[xt])gamedata[xt]=max(gamedata[xt],x);
	else gamedata[xt]=x;
	printf("This song's rating is : %.3lf\nAnd your best score is : %d\n",get_rating(rating[xt],x),gamedata[xt]);
} 
inline void input_change(){
	string nd;int x;node xt;letagain:
	system("cls");
	cout<<"song or song's rating ? or input @ to exit\n";
	getline(cin,s);
	...
	for(int i=0,sz=s.size();i<sz;++i)x=x*10+(s[i]^48);
	gamedata[xt]=x;
	printf("change achieve ! ! ! \nThis song's rating is : %.3lf\n",get_rating(rating[xt],x));
} 
...
inline void arcb30(){
	...
	if(s[i]=='.'){++i;ptt=ptt+(s[i]^48)*0.1+(i+1<n?(s[i+1]^48)*0.01:0);}
	fout.open("!a b30.txt",ios::out|ios::trunc);
	map<node,int>:: iterator it=gamedata.begin();
	for(;it!=gamedata.end();++it)qp.push(b30((*it).first,(*it).second));
	b30 bi;
	...
	cout<<"b30 : "<<setprecision(5)<<fixed<<best30/30<<"\n";
	cout<<"r10 : "<<setprecision(5)<<fixed<<r10<<"\n";
	cout<<"ptt : "<<setprecision(2)<<fixed<<ptt<<"\n";
	cout<<"ptt_max : "<<setprecision(5)<<fixed<<(best30+best10)/40<<"\n";
	cout<<"_______________\n\n";
	fout<<"b30 : "<<setprecision(5)<<fixed<<best30/30<<"\n";
	fout<<"r10 : "<<setprecision(5)<<fixed<<r10<<"\n";
	fout<<"ptt : "<<setprecision(2)<<fixed<<ptt<<"\n";
	fout<<"ptt_max : "<<setprecision(5)<<fixed<<(best30+best10)/40<<"\n";			
	fout.close();
}
inline string nd_change(string nd,double rating){
	...
	// else if(nd=="PRS")return "${\\color{green}"+s+"}$";
	if(nd=="BYD")return "$\\color[RGB]{200,5,20}{"+s+"}$";
	else if(nd=="FTR") return "$\\color[RGB]{118,41,81}{"+s+"}$";
	else if(nd=="PRS")return "$\\color[RGB]{149,180,77}{"+s+"}$";
	else if(nd=="PST") return "$\\color[RGB]{79,181,213}{"+s+"}$";
	else return "$\\color[RGB]{107,83,136}{"+s+"}$";
}
inline void get_md(){
	int tot=0;
	...
	fout<<setprecision(5)<<fixed<<best30/30<<"|";
	fout<<setprecision(5)<<fixed<<r10<<"|";
	fout<<setprecision(5)<<fixed<<(best30+best10)/40<<"|\n\n";
	fout<<"|song|score|song|score|song|score|\n|:-:|:-:|:-:|:-:|:-:|:-:|\n";
	if(qp.size()<=30){
		while(!qp.empty()){
			bi=qp.top();++tot;
...
signed main(){
	...
	while(1){ 
		...
		if(s=="1"){
			...
			map<node,int>:: iterator it=gamedata.begin();
			for(;it!=gamedata.end();++it){fout<<"<"<<(*it).first.name<<"> <"<<(*it).first.nd<<"> <";fout<<(*it).second<<">\n";}
			fout.close();
		}
		if(s=="2"){
			arcb30();
		...
		if(s=="3"){
			system("tools\\python\\python datas\\change.py");
			read_data();
			arcb30();
			get_md();
			puts("Press any key to continue ...");
